...
// =============================================================================
// ⚡ SPEED: HTTP Keep-Alive Agents
// Reuses TCP connections across requests on warm Vercel instances.
// Eliminates the TCP handshake overhead (5-50ms per request) on repeated calls.
// =============================================================================

const HTTP_AGENT  = new http.Agent({
  keepAlive:    true,
  maxSockets:   10,   // max concurrent sockets per host
  maxFreeSockets: 5,  // how many idle sockets to keep alive
  timeout:      60_000,
});
...
// Pre-configured axios instance with keep-alive baked in
...
// ⚡ SPEED FIX: Backoff reduced from 2s/4s → 1s/2s (HTTP_522_BACKOFF_MS)
// =============================================================================
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if ([522, 502, 503, 504].includes(status) && attempt < maxRetries) {
        // ⚡ SPEED: 1s, 2s — was 2s, 4s
        const wait = HTTP_522_BACKOFF_MS * (attempt + 1);
...
// ✅ FIX 2 APPLIED — Wide sibling context label
// ✅ FIX 3 APPLIED — Linked quality+format pair (atomic candidate)
// =============================================================================
...
// ⚡ SPEED: Uses AXIOS_SCRAPER_TIMEOUT_MS (20s) — small HTML page, doesn't need 50s
// =============================================================================
...
export async function solveHBLinks(url: string): Promise<HBLinksResult> {
  try {
    const response = await axiosClient.get<string>(url, {
      ...
      timeout:      AXIOS_SCRAPER_TIMEOUT_MS,  // ⚡ 20s — was 50s
      responseType: 'text',
    });
...
export async function solveHubCDN(url: string): Promise<HubCDNResult> {
  try {
    ...
    const finalResp = await axiosClient.get<string>(targetUrl, {
      headers:      MOBILE_HEADERS,
      timeout:      AXIOS_SCRAPER_TIMEOUT_MS,  // ⚡ 20s — was 50s
      responseType: 'text',
    });
...
// ✅ PORT FIX: Now correctly calls port 5000 (was broken at 5001 via config)
// Uses full AXIOS_TIMEOUT_MS (50s) — VPS Selenium can take 15-30s
// =============================================================================

export async function solveHubCloudNative(url: string): Promise<HubCloudNativeResult> {
  console.log(`[HubCloud] Starting VPS solver (port 5000): ${url}`);
  try {
    const resp = await axiosClient.get(`${HUBCLOUD_API}/solve?url=${encodeURIComponent(url)}`, {
      timeout: AXIOS_TIMEOUT_MS,  // 50s — VPS needs full budget
    ...
    return { status: 'error', message: `VPS API (port 5000) error: ${msg}` };
  }
}

// =============================================================================
// FUNCTION 8: solveGadgetsWebNative
// Uses full AXIOS_TIMEOUT_MS (50s) — Timer VPS Selenium takes 20-35s
// =============================================================================

export async function solveGadgetsWebNative(url: string): Promise<GadgetsWebResult> {
  console.log(`[GadgetsWeb] Starting VPS Timer solver (port 10000): ${url}`);
  try {
    const resp = await axiosClient.get(`${TIMER_API}/solve?url=${encodeURIComponent(url)}`, {
      timeout: AXIOS_TIMEOUT_MS,  // 50s — Selenium timer bypass takes 20-35s
      headers: { 'User-Agent': 'MflixPro/1.0' },
    });
