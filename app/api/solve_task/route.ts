...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // ─── ⚡ SPEED: Run direct links + first timer link TRULY in parallel ──
    // Both start at the same time via Promise.all — no sequential waiting.
    // Direct links (HBLinks/HubDrive/HubCloud) → fast, concurrent
    // Timer link #0 → first only, remaining get fresh relay windows
    const directPromises = directLinks.map((l: any) =>
      processLink(l, l.id, taskId, extractedBy),
    );

    const firstTimerPromise = timerLinks.length > 0
      ? (console.log(`[solve_task] Starting first timer link in parallel: ${timerLinks[0].link}`),
         processLink(timerLinks[0], timerLinks[0].id, taskId, extractedBy))
      : Promise.resolve(null);

    // ⚡ Both run concurrently — saves up to 20s vs sequential await
    const [directSettled, firstTimerResult] = await Promise.all([
